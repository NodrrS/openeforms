...
language and translates it to XPath. `@openeforms/validate` runs the SDK's
Schematron rules. `@openeforms/lineage` groups notices into procedures and
diffs corrigenda. Only the command-line interface is still to come.
...
| [`@openeforms/validate`](packages/validate) | ISO Schematron validation, no JVM and no XSLT step | **working, tested** |
| [`@openeforms/lineage`](packages/lineage) | Procedure index and notice-version diffing, with coverage accounting | **working, tested** |
